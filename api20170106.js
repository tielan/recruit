...
请求参数
	参数名	类型	参数说明
	resume_id	Int	简历id(值为0或空代表新建简历，其他值代表编辑简历)
	cn_name	String	姓名
	User_id	Int	用户id标识
	sex	string	性别
	...
	qzzw	string	求职职位
	qwxz	int	期望薪资(薪资范围的最小值)
	qwxz1	int	期望薪资(薪资范围的最大值)
	gzlx	string	工作类型(1:实习生,2: 兼职,3:全职,4:全/兼职)
	jyjl	string	教育经历
	gzjl	string	工作经历
	zwpj	String	自我介绍
	jntcms	string	技能特长
...
请求参数	参数名	类型	参数说明
	personal_id	Int	用户id
	status	int	1被查看，2企业接收，3企业邀请
返回结果
JSON格式	属性名	类型	属性说明
	result	Int	状态标示：1成功。其于失败
	msg	String	对应状态消息
	data	String	返回列表json串
	JOB_ID	int	公司发布的岗位信息ID
...
返回结果
JSON格式	属性名	类型	属性说明
	result	Int	状态标示：1成功。1无提货单信息其于失败
	msg	String	对应状态消息
	...
	addr_area	String	公司区域
	address	String	公司地址
	post_type getPersonSendResumeInfo	String	岗位类型（1:全职，2:兼职，3:实习）
	education_area	String	学历要求
	post_introduction	String	职位介绍
	post_demand	String	职位要求
	company_introduction	String	公司介绍
...
JSON属性	属性名	类型	属性说明
	resume_id	Int	简历id
	post_id	int	公司发布的岗位信息ID
	post_name	string	岗位名称
	company_id	int	公司ID
	company_name	string	公司名称
	create_time	string	投递时间
返回结果	返回格式：
{
    "msg": "请求成功",
    "data": [{ 
...
请求类型	get
请求参数	参数名	类型	参数说明
	company_id	Int	公司id(可选请求参数)
	user_id	int	用户ID（必须的请求参数）
返回结果
JSON属性	属性名	类型	属性说明
	tile	string	企业邀请通知标题
	status	String	用户简历状态（1：已接收，2：已查看,3:企业邀请）
	post_id	int	公司发布的岗位信息ID
	post_name	string	岗位名称
	company_id	int	公司ID
	company_name	string	公司名称
...
接口方向	手机客户端 ->服务器系统
接口地址	http://222.240.214.122:18000/mobile_interfaces/mobile_info/ getPushInfo.do
请求类型	get
请求参数	参数名	类型	参数说明
	company_id	Int	公司id(可选请求参数)
	user_id	int	个人ID（必须的请求参数）
返回结果
JSON属性	属性名	类型	属性说明
	post_id	int	公司发布的岗位信息ID
	post_name	String	岗位名称
...
{"birthday":"1994/10/16",
"sex":"2","cn_name":"谢曼林",
"cardno":"43022319620403655122","personal_id":"9106"}