...
class MyResumeListPage extends React.Component {
    constructor(props) {
        super(props);
    ...
    componentDidMount() {
        InteractionManager.runAfterInteractions(() => {
            ...
            dispatch(getMyResumeListAction(userInfo.personal_id));
        });
                ...
                <View style={{ flex: 1 }}>
                    {
                        (MyResumeList.listData._cachedRowCount > 0) ?
                            <ListView
                                dataSource={MyResumeList.listData}
...
class MyResumeListContainer extends Component {

    render() {
        return (
            <MyResumeListPage {...this.props} />
        );
    }
}

function mapStateToProps(state) {
    const { MyResumeList } = state;
    return {
        MyResumeList,
    }
}

export default connect(mapStateToProps)(MyResumeListContainer);